...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "Elevated Everyday Living — Discover. Solve. Shop.",
    description:
      "Practical guides, beauty discoveries, problem-solving finds and destination ideas to help you make better everyday choices.",
    type: "website",
  },
...
const PILLARS = [
  ...
  },
  {
    href: "/hair",
    label: "Haircare",
    hook: "Frizz, dryness & routine fixes",
    emoji: "💆",
  ...
  },
  {
    ...
    emoji: "🛠️",
    color: "from-brand-50 to-stone-50 border-brand-200",
  ...
  },
  {
    href: "/travel",
    label: "Travel",
    hook: "What to wear & pack for every destination",
    emoji: "✈️",
...
// ─── Sample "Solve This" problem cards ────────────────────────────────────────
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-brand-100 via-brand-50 to-white px-4 sm:px-6 pt-16 pb-20 text-center">
        ...
        <p className="text-lg text-brand-700 max-w-xl mx-auto mb-10 leading-relaxed">
          Practical guides, beauty discoveries, outfit ideas and useful finds — to help you make better everyday choices.
        </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {PILLARS.map((p) => (
            <Link
              key={p.href}
              href={p.href}
              className={`group flex flex-col items-center text-center p-4 rounded-xl border bg-gradient-to-b ${p.color} hover:shadow-soft transition-all`}
            >
              <span className="text-3xl mb-2">{p.emoji}</span>
              <span className={`font-semibold text-sm ${p.textColor}`}>{p.label}</span>
              <span className="text-xs text-brand-500 mt-1 leading-tight hidden sm:block">{p.hook}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* ── Featured articles (dynamic, only if content exists) ── */}
      {featured.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
      ...
      {/* ── What To Wear / Travel teaser ── */}
      <section className="bg-gradient-to-r from-sky-50 to-blue-50 border-y border-sky-100 py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <span className="text-xs font-semibold tracking-widest text-sky-600 uppercase">Travel & Style</span>
              ...
              <p className="text-sky-800 text-sm leading-relaxed max-w-lg">
                From Manali in June to Goa in winter — destination-specific outfit guides that tell you exactly what to pack.
              </p>
            </div>
      ...
      <section className="bg-brand-950 text-white py-14 px-4 sm:px-6 text-center">
        ...
        <p className="text-brand-300 max-w-md mx-auto text-sm leading-relaxed mb-8">
          Every guide here is designed to answer exactly what the pin promised. No fluff. No bait-and-switch.
        </p>
        <Link
          href="/how-we-choose"
...
// ─── Reusable article card component (page-level, server only) ────────────────

function ArticleCard({ article }: { article: { slug: string; pillar: string; title: string; description: string; category: string } }) {
  ...
  return (
    ...
    >
      ...
      <div className="aspect-[16/9] bg-brand-100 flex items-center justify-center text-brand-400 text-sm font-medium">
        {article.category}
      </div>
