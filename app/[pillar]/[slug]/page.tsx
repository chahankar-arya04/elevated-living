...
export default function ArticlePage({
  ...
  // Try to find products tagged to this article's category/tags
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
      ...
      <header className="mb-8">
        ...
        <div className="flex items-center gap-3 text-sm text-brand-500">
          ...
          {article.lastUpdated && (
            <>
              <span>·</span>
              <span>Updated {new Date(article.lastUpdated).toLocaleDateString("en-IN", { month: "short", year: "numeric" })}</span>
            </>
          )}
        </div>
      </header>
      ...
      {article.heroImage ? (
        ...
        <div className="w-full rounded-xl mb-8 aspect-[16/9] bg-brand-100 flex items-center justify-center text-brand-400 text-sm">
          {article.category}
        </div>
      )}
      ...
      <div className="text-xs text-brand-500 bg-brand-50 border border-brand-200 rounded-md px-4 py-2.5 mb-8">
        <strong>Disclosure:</strong> This article may contain affiliate links. If you make a purchase through our links, we may earn a small commission at no extra cost to you.{" "}
        <Link href="/affiliate-disclosure" className="underline hover:text-brand-700">Learn more.</Link>
      </div>

      {/* ── Article content ── */}
      <div
        className="prose prose-brand max-w-none text-brand-800 prose-headings:font-serif prose-headings:text-brand-900 prose-a:text-brand-700 prose-a:underline hover:prose-a:text-brand-900 prose-strong:text-brand-900"
      ...
      {relevantProducts.length > 0 && (
        <section className="mt-12 pt-10 border-t border-brand-200">
          ...
          <p className="text-sm text-brand-600 mb-6">
            Carefully selected finds relevant to this guide. We only list products that pass our safety and quality review.
          </p>
      ...
      <div className="mt-12 pt-10 border-t border-brand-200 text-center">
        <p className="text-sm text-brand-600 mb-4">
          Found this helpful? Explore more guides:
        </p>
        <div className="flex flex-wrap gap-3 justify-center">
          <Link href="/skin" className="px-4 py-2 bg-rose-50 text-rose-800 border border-rose-200 rounded-full text-xs font-medium hover:bg-rose-100 transition-colors">✨ Skincare</Link>
          <Link href="/hair" className="px-4 py-2 bg-amber-50 text-amber-800 border border-amber-200 rounded-full text-xs font-medium hover:bg-amber-100 transition-colors">💆 Haircare</Link>
          <Link href="/travel" className="px-4 py-2 bg-sky-50 text-sky-800 border border-sky-200 rounded-full text-xs font-medium hover:bg-sky-100 transition-colors">✈️ Travel</Link>
          <Link href="/trending" className="px-4 py-2 bg-orange-50 text-orange-800 border border-orange-200 rounded-full text-xs font-medium hover:bg-orange-100 transition-colors">🔥 Trending</Link>
        </div>
      </div>
    </article>
  );
