...
export const PILLAR_META: Record<
  string,
  { label: string; description: string; seoTitle: string; color: string; emoji: string }
> = {
  skin: {
    label: "Skincare",
    description:
  ...
  "useful-finds": {
    label: "Useful Finds",
    description:
      "Small tools and products that solve surprisingly annoying everyday problems. From organisation to beauty to travel — if it genuinely helps, it's here.",
    seoTitle: "Useful Finds — Products That Solve Real Problems",
    color: "from-brand-50 to-stone-50",
    emoji: "🛠️",
  },
  style: {
    label: "Style",
...
export default function PillarIndexPage({ params }: { params: { pillar: string } }) {
  ...
  return (
    <div className={`bg-gradient-to-b ${meta.color} to-white min-h-screen`}>
      {/* Header */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-12 pb-10">
        <div className="flex items-center gap-3 mb-4">
          <span className="text-4xl">{meta.emoji}</span>
          <div>
            <nav className="text-xs text-brand-500 mb-1">
              <Link href="/" className="hover:text-brand-700">Home</Link>
              {" › "}
              <span className="text-brand-800 font-medium">{meta.label}</span>
            </nav>
            <h1 className="text-3xl sm:text-4xl font-serif font-bold text-brand-900">
              {meta.label}
            </h1>
          </div>
        </div>
